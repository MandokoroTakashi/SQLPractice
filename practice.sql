...
SELECT * FROM countries WHERE indep_year < 1900 OR population > 100000; 
...
SELECT c.name, l.language FROM countries c LEFT OUTER JOIN countrylanguages l ON c.code = l.country_code;

-- 問26
-- 全ての国と言語と市区町村を表示してください。
SELECT c.name, t.name, l.language FROM countries c LEFT OUTER JOIN cities t ON c.code = t.country_code LEFT OUTER JOIN countrylanguages l ON c.code = l.country_code;

-- 問27
-- 全ての有名人を出力してください。左側外部結合を使用して国名なし（country_codeがNULL）も表示してください。
SELECT e.name, c.name FROM celebrities e LEFT OUTER JOIN countries c ON e.country_code = c.code;

-- 問28
-- 全ての有名人の名前,国名、第一言語を出力してください。
SELECT e.name, c.name, l.language FROM celebrities e LEFT OUTER JOIN countries c ON e.country_code = c.code LEFT OUTER JOIN countrylanguages l ON e.country_code = l.country_code WHERE l.percentage = (SELECT MAX(percentage) FROM countrylanguages l2 WHERE l2.country_code = e.country_code);

-- 問29
-- 全ての有名人の名前と国名をに出力してください。 ただしテーブル結合せずサブクエリを使用してください。
SELECT e.name, (SELECT c.name FROM countries c WHERE e.country_code = c.code) AS "国名" FROM celebrities e;

-- 問30
-- 最年長が50歳以上かつ最年少が30歳以下の国を表示させてください。
SELECT country_code, MAX(ce.age), MIN(ce.age) FROM celebrities ce WHERE country_code != "" GROUP BY country_code HAVING MAX(ce.age) >= 50 AND MIN(ce.age) <= 30;

-- 問31
-- 1991年生まれと、1981年生まれの有名人が何人いるか調べてください。ただし、日付関数は使用せず、UNION句を使用してください。
SELECT "1980" AS "誕生年", count(birth) AS "人数" FROM celebrities WHERE birth >= '1980-01-01 00:00:00' AND birth <= '1980-12-31 23:59:59' UNION SELECT "1981" AS "誕生年", count(birth) AS "人数" FROM celebrities WHERE birth >= '1981-01-01 00:00:00' AND birth <= '1981-12-31 23:59:59';

-- 問32
-- 有名人の出身国の平均年齢を高い方から順に表示してください。ただし、FROM句はcountriesテーブルとしてください。
SELECT c.name AS "国名", avgage AS "平均年齢" FROM countries c LEFT OUTER JOIN (SELECT country_code, AVG(age) AS avgage FROM celebrities GROUP BY country_code) ce ON c.code = ce.country_code WHERE avgage IS NOT NULL ORDER BY avgage DESC;